...
interface FilterComponentProps {
  metaDatas: { key: string; name: string; values: string[] }[];
  items: string[];
  open: boolean;
  onClose: () => void;
  onClear: () => void;
    ...
    <Drawer open={open} onClose={onClose} anchor="right">
      <Box sx={{ width: 480 }} role="presentation">
        ...
        >
          <Typography
            variant="h2"
            sx={{ color: "#191919", fontWeight: 500, fontSize: "24px" }}
          >
            Filters
          </Typography>
          <IconButton
            onClick={onClose}
            sx={{ color: "black", padding: "0px !important" }}
          >
        ...
        >
          <Typography
            variant="h6"
            sx={{ color: "#191919", fontWeight: 500, fontSize: "16px" }}
          >
            Status
          </Typography>
        ...
        >
          ...
          <Box display="flex" gap={2} flexWrap="wrap" alignItems="flex-start">
            <FormControl sx={{ minWidth: 200 }}>
              ...
              <Select
                ...
                value={selectedKey}
                onChange={(e) => {
                  const selected = metaDatas.find(
                    (m) => m.key === e.target.value
                  );
                  setSelectedKey(selected?.key ?? null);
                  setSelectedKeySupportedValues(selected?.values ?? []);
                  setSelectedValue("");
                }}
                label="Key"
            ...
            <FormControl sx={{ minWidth: 200 }} disabled={!selectedKey}>
              ...
              <Select
                ...
                label="Value"
                onClose={() => {
                  if (selectedValue) handleValueChange({ target: { value: selectedValue } } as any);
                }}
                sx={{ borderRadius: 2, border: "1px solid #CECFD2" }}
              >
                {selectedKey &&
                  selectedKeySupportedValues?.map((value) => (
          ...
          <Box display="flex" flexWrap="wrap" gap={1} mt={3}>
            {tags.map((tag, index) => {
              const meta = metaDatas.find((m) => m.key === tag.key);
              return (
                <Chip
                  key={`${tag.key}-${tag.value}-${index}`}
                  label={`${meta?.name}: ${tag.value}`}
                  variant="outlined"
...
//                     setSelectedKeySupportedValues(
//                       metaDatas.find((m) => m.id === e.target.value)
//                         .supported_values
//                     );
//                     setSelectedValue("");
//                   }}
...
//             <Box display="flex" flexWrap="wrap" gap={1} mt={3}>
//               {tags.map((tag, index) => {
//                 const metaDataLabel = metaDatas.find(
//                   (meta) => meta.id === tag.key
//                 ).label;
//                 return (
//                   <Chip
//                     key={`${tag.key}-${tag.value}-${index}`}

...
function Reports() {
  ...
  useEffect(() => {
    async function fetchData() {
      const orgId = JSON.parse(Cookies.get("user") ?? "")?.orgId;
      if (orgId) {
        const processData = await getProcessList(orgId);
        const formatted = transformProcessData(processData.data);
        setProcessData(formatted);

        const assetData = await getOrganizationAssets(orgId);
  ...
  return (
    <>
      {JSON.parse(Cookies.get("user") ?? "")?.org_id ? (

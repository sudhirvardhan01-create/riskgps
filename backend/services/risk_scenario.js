...
class RiskScenarioService {
    static async createRiskScenario(data) {
        ...
        return await sequelize.transaction(async (t) => {
            ...
            console.log("[createRiskScenario], risk scenario mapped values", riskScenarioData);

            const scenario = await RiskScenario.create(riskScenarioData, { transaction: t });

            await this.handleRiskScenarioProcessMapping(scenario.id, data.related_processes ?? [], t);

            await this.handleRiskScenarioAttributes(scenario.id, data.attributes ?? [], t);

            return scenario;
        });
    }

    static async getAllRiskScenarios(page = 0, limit = 6,  searchPattern = null, sortBy = 'created_at', sortOrder = 'ASC', statusFilter = [], attrFilters = []) {
        ...
        const whereClause = this.handleRiskScenarioFilters(searchPattern, statusFilter, attrFilters);
    ...
    static async updateRiskScenario(id, data) {
        ...
        return await sequelize.transaction(async (t) => {
            ...
            const updatedRiskScenario = await scenario.update( riskScenarioData, { transaction: t } );
    ...
    static async handleRiskScenarioProcessMapping (riskScenarioId, relatedProcesses, transaction) {
    ...
    static async handleRiskScenarioAttributes(riskScenarioId, attributes, transaction) {
    ...
    static handleRiskScenarioFilters(searchPattern = null, statusFilter = [], attrFilters = [] ) {
        let conditions = [];

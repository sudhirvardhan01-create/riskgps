...
class OrganizationService {
  ...
  static async getAssetsByOrgIdV2(orgId) {
    try {
      ...
      let assets = await OrganizationAsset.findAll({
        where: {
          organizationId: orgId,
          isDeleted: false,
        },
      ...
      reportsTableData.map((item) => {
        ...
        if (assetId && !assetMap.has(assetId)) {
          assetMap.set(assetId, item.toJSON());
        }
      });
      ...
      for (const asset of assets) {
        ...
        asset.assetCriticality =
          assetData?.aggAssetResidualRiskLevelRiskDashboardCIOTab ?? null;
      }
      return assets;
    } catch (err) {
      throw new CustomError(
  ...
  static async syncSingleLibrary(
    name,
    organizationId,
    orgBusinessUnitId,
    userId,

...
const ReportsService = require("./reports_service");
...
class SyncupService {
  ...
  static riskDashboardCIOTab(dataArray, assetControlScores, riskAppetite) {
    ...
    return dataArray.map((item) => {
      ...
      const residualImpactInMillionDollarsRiskDashboardCIOTab =
        roundToOneDecimal(rawResidualImpactRiskDashboardCIOTab);
  ...
  static async getLastSyncupDetails(orgId) {
    ...
    let result = await models.sequelize.query(
      `
        SELECT 
          COUNT(DISTINCT business_unit) AS "businessUnits",
          COUNT(DISTINCT business_process) AS "businessProcesses",
          COUNT(DISTINCT risk_scenario) AS "riskScenarios",
          COUNT(DISTINCT asset) AS "assets"
        FROM reports_master
        WHERE org_id = :orgId
              AND updated_at = :latestTimeStamp
      `,
      {
        replacements: { orgId, latestTimeStamp },

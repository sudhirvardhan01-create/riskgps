...
class MitreThreatControlService {
  ...
  static async updateMitreThreatControl(payload) {
    ...
    if (
      ...
      payload.controls?.length < 1
    ) {
      throw (
        (new CustomError("Controls array required"), HttpStatus.BAD_REQUEST)
      );
    }

    const incomingIds = (payload.controls || [])
      .map((c) => c.id)
      .filter(Boolean);
    ...
    if (payload.controls && Array.isArray(payload.controls)) {
      for (const control of payload.controls) {
        ...
        if (control.id) {
          ...
          await MitreThreatControl.update(controlData, {
            where: { id: control.id },
          });
        } else {
          // Create new control
